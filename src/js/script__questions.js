...
export function changeQuestionpic(idQuestion){
    ...
    answers_img.forEach((answer_img)=>{
        if(i==j){
            answer_img.style.backgroundImage = "src/url('images/full/"+images[idQuestion]['imageNum']+"full.jpg')";
        }else{
            answer_img.style.backgroundImage = "src/url('images/full/"+res[j]['imageNum']+"full.jpg')";
        }
        j++
    })
    overlay_popup.classList.remove('overlay__show');
